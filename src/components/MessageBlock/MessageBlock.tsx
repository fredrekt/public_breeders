...
const MessageBlock: React.FC<MessageBlockProps> = ({ message, right }) => {
	return (
		<div data-message-date={message.createdAt} className={`messageBlock ${right ? 'rightAlign' : ''}`}>
			<Avatar
				src={
				...
				<div className="messageBlockMetaInfo">
					{message.sender && (
						<b className="messageBlockUser">
							{message.sender.firstName} {message.sender.lastName} {right ? '(You)' : ''}
						</b>
					)}
				</div>
				<div className="messageBlockMsg">
					<Typography.Paragraph className="messageBlockTxt">{message.message}</Typography.Paragraph>
				</div>
